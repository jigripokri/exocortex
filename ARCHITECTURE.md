...
**Step 0.5 — Data source health check.** Ping every MCP in parallel, record UP/DOWN. If anything is unreachable, surface a `⚠️ DEGRADED` banner in the email. Never block the briefing — a degraded briefing is better than no briefing.

**Step 1 — Memory load.** Read the seven memory files. This is the operator's context for everything that follows.

**Step 2..N — Scan inputs.** WhatsApp groups, DMs, Gmail, Calendar, Tasks, SMS, Fitbit, Monarch — whatever this briefing needs. Always parallelize. Apply tier weights so high-signal threads surface first.
...
|--------|--------|----------------|
| **WhatsApp** | Native MCP | Most personal conversation lives here for non-US users; commitments hide in here |
...
| **Fitbit** | Plugin with OAuth | Sleep, HR, workouts — input to health.md |
| **Monarch Money** | Local MCP server (read-only) | Source for the monthly financial sweep |

Full setup in [`docs/data-sources.md`](./docs/data-sources.md). Known gotchas (especially WhatsApp's LID migration) in [`docs/whatsapp-gotchas.md`](./docs/whatsapp-gotchas.md).
